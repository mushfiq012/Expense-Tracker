...
from src.expense_op import save_parameter

logo_image = Image.open("img/company_logo.png")
ai_image = Image.open("img/company_banner.png")

st.set_page_config(page_title="Grow with Data", page_icon=logo_image, layout="wide",initial_sidebar_state="expanded")
...
with col2:
    st.markdown("# Welcome to Grow with Data")
...
def menu():
    ...
    task = st.sidebar.selectbox('SELECT',
                                ('>>>>>>>>',
                                 'Save Expense Record',
                                 'Parameter Insertion',
                                 'Reporting'))
    ...
    if task == 'Save Expense Record':
        ...
        # save_parameter(cursor1, db)

@login
def main():
    cols1, cols2, cols3 = st.columns((1, 4, 1))
    cols2.markdown("<h1 style='text-align: left;margin-top:-2rem; margin-left:1rem; color: #008080;'>Personal Expense Tracker</h1>", unsafe_allow_html=True)
    menu()
